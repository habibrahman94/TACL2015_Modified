    ...
    if "how" in [x[0].lower() for x in words]:
        ...
        if len(targets)==1:
            ...
            if words[tidx][1]["PartOfSpeech"] in ["NN","NNS"]:
                ...
                good = 0
                if t == "more":
                    targets = [x[1] for x in deps if x[2].rsplit("-",1)[0] in ['more']]
                    if targets:
                        t,tidx = targets[0].rsplit("-",1)
                        tidx = int(tidx)-1
                        if words[tidx][1]["PartOfSpeech"] in ["NN","NNS"]:
                    ...
                    targets = [x[2] for x in deps if x[1].rsplit("-",1)[0] in ['did'] and x[0]=='nsubj']
                    if targets:
                        t,tidx = targets[0].rsplit("-",1)
                        tidx = int(tidx)-1
                        if words[tidx][1]["PartOfSpeech"] in ["NN","NNS"]:
